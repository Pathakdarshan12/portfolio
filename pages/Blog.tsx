...
import {
  ...
  Sparkles, Filter, ChevronRight, Hash
} from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
            <div className="max-w-2xl">
              ...
              <h1 className="text-5xl sm:text-7xl font-extrabold tracking-tight mb-6 text-left">
                ...
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary-500 to-indigo-500">Platform Edge</span>
              </h1>
              <p className="text-lg text-slate-600 dark:text-slate-400 text-left">
                Architectural deep-dives, modern data stack tutorials, and tactical guides for building reliable data infrastructure.
              </p>
            </div>
          </div>
        </header>
        ...
        {!search && selectedCategory === 'All' && featuredPost && (
          <div className="mb-24 transition-all duration-500">
            <Link to={`/blog/${featuredPost.slug}`} className="group relative block bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[3rem] overflow-hidden shadow-2xl hover:shadow-primary-500/10 transition-all duration-500">
                ...
                <div className="p-8 lg:p-16 flex flex-col justify-center">
                  ...
                  <div className="flex items-center justify-between mt-auto">
                    <div className="flex items-center gap-4">
                      <img src={featuredPost.author.avatar} className="w-12 h-12 rounded-full border-2 border-primary-500/20" alt="" />
                      <div className="text-left">
                        <div className="text-sm font-bold">{featuredPost.author.name}</div>
                        <div className="text-xs text-slate-400 font-medium">{featuredPost.date}</div>
                      </div>
                    </div>
            ...
            >
              ...
              <div className="p-10 flex-grow flex flex-col">
                <div className="flex items-center gap-6 text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-6">
                  ...
                  <span className="flex items-center"><Clock size={14} className="mr-2" /> {post.readingTime}</span>
                </div>
                ...
                <div className="mt-auto flex justify-between items-center pt-8 border-t border-slate-100 dark:border-slate-800">
                  <div className="flex flex-wrap gap-2">
                    {post.tags.slice(0, 2).map(t => (
                      <span key={t} className="text-[10px] font-bold text-primary-500 uppercase tracking-widest">#{t}</span>
                    ))}
                  </div>
                  <Link
                    to={`/blog/${post.slug}`}
